...
bool LoginService::Start(GameTitle::Asio::IoContext* ioContext)
{
    _ioContext = ioContext;
    _bindIP = sConfigMgr->GetStringDefault("BindIP", "0.0.0.0");
    _port = sConfigMgr->GetIntDefault("Login.Port", 8081);
    ...
    std::string configuredAddress = sConfigMgr->GetStringDefault("Login.ExternalAddress", "127.0.0.1");
    Optional<boost::asio::ip::tcp::endpoint> externalAddress = GameTitle::Net::Resolve(resolver, boost::asio::ip::tcp::v4(), configuredAddress, std::to_string(_port));
    if (!externalAddress)
    {
        TC_LOG_ERROR("server.rest", "Could not resolve Login.ExternalAddress %s", configuredAddress.c_str());
        return false;
    }

    _externalAddress = *externalAddress;

    configuredAddress = sConfigMgr->GetStringDefault("Login.LocalAddress", "127.0.0.1");
    Optional<boost::asio::ip::tcp::endpoint> localAddress = GameTitle::Net::Resolve(resolver, boost::asio::ip::tcp::v4(), configuredAddress, std::to_string(_port));
    if (!localAddress)
    {
        TC_LOG_ERROR("server.rest", "Could not resolve Login.LocalAddress %s", configuredAddress.c_str());
        return false;
    }

    _localAddress = *localAddress;
    _localNetmask = GameTitle::Net::GetDefaultNetmaskV4(_localAddress.address().to_v4());

    // set up form inputs
    LoginServerName::JSON::Login::FormInput* input;
    _formInputs.set_type(LoginServerName::JSON::Login::LOGIN_FORM);
    ...
    _loginTicketDuration = sConfigMgr->GetIntDefault("Login.TicketDuration", 3600);

    _thread = std::thread(std::bind(&LoginService::Run, this));
    return true;
}
...
boost::asio::ip::tcp::endpoint const& LoginService::GetAddressForClient(boost::asio::ip::address const& address) const
{
    ...
    if (GameTitle::Net::IsInNetwork(_localAddress.address().to_v4(), _localNetmask, address.to_v4()))
        return _localAddress;
...
void LoginService::Run()
{
    ...
    _getHandlers["/loginserver/login/"] = &LoginService::HandleGetForm;
    _getHandlers["/loginserver/gameAccounts/"] = &LoginService::HandleGetGameAccounts;
    _getHandlers["/loginserver/portal/"] = &LoginService::HandleGetPortal;

    _postHandlers["/loginserver/login/"] = &LoginService::HandlePostLogin;
    _postHandlers["/loginserver/refreshLoginTicket/"] = &LoginService::HandlePostRefreshLoginTicket;
    ...
    soapServer.ctx = LoginServerName::SslContext::instance().native_handle();
    ...
    while (!_stopped)
    {
        ...
        GameTitle::Asio::post(*_ioContext, [soapClient]()
        {
...
int32 LoginService::HandleHttpRequest(soap* soapClient, char const* method, HttpMethodHandlerMap const& handlers)
{
    ...
    if (handler != handlers.end())
    {
        ...
        if (status != SOAP_OK)
        {
            ...
            return SendResponse(soapClient, LoginServerName::JSON::Login::ErrorResponse());
        }
...
int32 LoginService::HandleGetForm(std::shared_ptr<AsyncRequest> request)
{
    return SendResponse(request->GetClient(), _formInputs);
}

int32 LoginService::HandleGetGameAccounts(std::shared_ptr<AsyncRequest> request)
{
    if (!request->GetClient()->userid)
        return 401;

    request->SetCallback(GameTitle::make_unique<QueryCallback>(LoginDatabase.AsyncQuery([&] {
        ...
        .WithPreparedCallback([this, request](PreparedQueryResult result)
    {
        ...
        if (result)
        {
            ...
            do
            {
                ...
                LoginServerName::JSON::Login::GameAccountInfo* gameAccount = response.add_game_accounts();
    ...
    GameTitle::Asio::post(*_ioContext, [this, request]() { HandleAsyncRequest(request); });

    return SOAP_OK;
}

int32 LoginService::HandleGetPortal(std::shared_ptr<AsyncRequest> request)
{
    boost::asio::ip::tcp::endpoint const& endpoint = GetAddressForClient(boost::asio::ip::address_v4(request->GetClient()->ip));
    std::string response = GameTitle::StringFormat("%s:%d", endpoint.address().to_string().c_str(), sConfigMgr->GetIntDefault("LoginServerNamePort", 1119));
...
int32 LoginService::HandlePostLogin(std::shared_ptr<AsyncRequest> request)
{
    char* buf = nullptr;
    size_t len = 0;
    soap_http_body(request->GetClient(), &buf, &len);

    LoginServerName::JSON::Login::LoginForm loginForm;
    if (!buf || !JSON::Deserialize(buf, &loginForm))
    {
        ResponseCodePlugin::GetForClient(request->GetClient())->ErrorCode = 400;

        LoginServerName::JSON::Login::LoginResult loginResult;
        loginResult.set_authentication_state(LoginServerName::JSON::Login::LOGIN);
    ...
    request->SetCallback(GameTitle::make_unique<QueryCallback>(LoginDatabase.AsyncQuery(stmt)
        .WithChainingPreparedCallback([request, login, sentPasswordHash, this](QueryCallback& callback, PreparedQueryResult result)
    {
        if (result)
        {
            ...
            if (sentPasswordHash == pass_hash)
            {
                ...
                {
                    LoginServerName::JSON::Login::LoginResult loginResult;
                    loginResult.set_authentication_state(LoginServerName::JSON::Login::DONE);
...
int32 LoginService::HandlePostRefreshLoginTicket(std::shared_ptr<AsyncRequest> request)
{
    if (!request->GetClient()->userid)
        return 401;
    ...
    request->SetCallback(GameTitle::make_unique<QueryCallback>(LoginDatabase.AsyncQuery([&] {
        ...
        .WithPreparedCallback([this, request](PreparedQueryResult result)
    {
        LoginServerName::JSON::Login::LoginRefreshResult loginRefreshResult;
        if (result)
        {
...
void LoginService::HandleAsyncRequest(std::shared_ptr<AsyncRequest> request)
{
    ...
    else if (request->GetResponseStatus())
    {
        ...
        SendResponse(request->GetClient(), LoginServerName::JSON::Login::ErrorResponse());
    }
}
...
int32 LoginService::ResponseCodePlugin::Copy(soap* /*s*/, soap_plugin* dst, soap_plugin* src)
{
...
LoginService::ResponseCodePlugin* LoginService::ResponseCodePlugin::GetForClient(soap* s)
{
    return ASSERT_NOTNULL(reinterpret_cast<ResponseCodePlugin*>(soap_lookup_plugin(s, PluginId)));
}

char const* const LoginService::ContentTypePlugin::PluginId = "bnet-content-type";

int32 LoginService::ContentTypePlugin::Init(soap* s, soap_plugin* p, void* arg)
{
...
void LoginService::ContentTypePlugin::Destroy(soap* s, soap_plugin* p)
{
...
int32 LoginService::ContentTypePlugin::OnSetHeader(soap* s, char const* key, char const* value)
{
